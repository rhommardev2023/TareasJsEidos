...
/* 2 - Esta función crearHTMLPokemon(pokemon) crea un el código HTML de un 
elemento <li> con los datos del Pokemon que reciba como parámetro.
 Aquí hay herramientas de JS que veremos en la Clase 10.
--> createElement('nombreElementoHtml') es un método del Documento HTML 
que crea un elemento HTML cualquiera.
--> appendChild(item) es un método que tienen los elementos HTML y permite 
"agregarles un elemento hijo".
Donde dice listItem.appendChild(nombrePokemon);
 Esto le agrega al <li> el <h3> con el nombre del Pokemon.
Su tarea:
Al mismo <li> debemos agregarle la imagen y el tipo de Pokemon.
 Para esto repitan esa última línea de código 2 veces más pero pasando 
 como argumento al método appendChild() primero imagenPokemon y luego
  tipoPokemon
*/
...
/* 3 - Con este forEach() recorremos el array pokemons y a partir de 
cada elemento creamos un pokemon y lo insertamos en el HTML dentro 
del <ul>
Su tarea:
Dentro de la función, falta crear una variable llamada 
pokemonListItem y asignarle un nuevo pokemon llamando a la 
funcion crearHTMLPokemon(pokemon)
